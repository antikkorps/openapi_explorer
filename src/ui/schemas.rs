...
pub fn render_schemas_view(f: &mut Frame, app: &mut App, chunks: Vec<Rect>) {
    ...
    let schema_items: Vec<ListItem> = app.filtered_schemas
        .iter()
        .map(|schema| {
    ...
    let schemas_list = List::new(schema_items)
        .block(crate::ui::layout::panel_block("Schemas", app.current_panel == Panel::Left))
        ...
        if let Some(schema) = app.field_index.schemas.get(selected_schema) {
            ...
            let mut details_text = vec![
                Line::from(vec![
                    ...
                    Span::styled(selected_schema, Style::default().add_modifier(Modifier::BOLD)),
                ]),
                Line::from(""),
                Line::from(vec![
            ...
            for (i, field) in fields.iter().enumerate() {
                let field_type = schema.get_field_type(field).unwrap_or_else(|| "unknown".to_string());
                details_text.push(Line::from(vec![
                    Span::styled(format!("  {}. ", i + 1), Style::default().fg(Color::DarkGray)),
                    Span::styled(field, Style::default().add_modifier(Modifier::BOLD)),
                    Span::styled(format!(" ({})", field_type), Style::default().fg(Color::Green)),
                ]));
            }

            let details_widget = Paragraph::new(details_text)
                .wrap(Wrap { trim: true })
                .block(crate::ui::layout::panel_block("Schema Details", app.current_panel == Panel::Center));
    ...
    if let Some(selected_schema) = &app.selected_schema {
        ...
        let no_endpoints = Paragraph::new("Select a schema to see related endpoints")
            .style(Style::default().fg(Color::DarkGray))
            .block(crate::ui::layout::panel_block("Related Endpoints", app.current_panel == Panel::Right));
